...
class ConnectionHandle(threading.Thread):
    ...
    def run(self):
        ...
        #     print(f"{datetime.now().strftime('%d %b %H:%M:%S')} - >>>  {req.method:<8} {req.path} {req.protocol} BLOCKED")
        #     return
        ...
        try:
            ...
            self.client_conn.close()
            print(f"{datetime.now().strftime('%d %b %H:%M:%S')} - >>>  {req.method:<8} {req.path} {req.protocol} SERVICE UNAVAILABLE")
            return
        ...
        while True:
            ...
            try:
                ...
                if self.server_conn in triple:
                    ...
                    if not res:
                        res = Response(data)
                        print(f"{datetime.now().strftime('%d %b %H:%M:%S')} - >>>  {req.method:<8} {req.path} {req.protocol} {res.status if res else ''}")
                    if not data:
                        break
...
class Server:
    def __init__(self, host: str, port: int):
        print(f"{datetime.now().strftime('%d %b %H:%M:%S')} - >>>  Proxy server starting")
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.bind((host, port))
        self.sock.listen(BACKLOG)
        print(f"{datetime.now().strftime('%d %b %H:%M:%S')} - >>>  Listening at: http://{host}:{port}")
...
if __name__ == '__main__':
    ser = Server(host="127.0.0.1", port=8009)
    ser.start()
